...
static t_buffer *hello_functionData (t_gobj *z, int flags)
{
    t_hello *x = (t_hello *)z;

    if (x->x_keep || object_isUndoOrEncaspulate (flags)) {
    //
    t_buffer *b = buffer_newData();     /* Buffer correctly labeled. */
...
static void hello_functionDismiss (t_gobj *z)
{
